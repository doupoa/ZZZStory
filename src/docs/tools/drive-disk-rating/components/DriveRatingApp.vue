    ...
    const characterData = {
      ...
      driveDiscs: (item.driveDiscs || []).map(disc => {
        ...
        return {
          ...
          invalidProperty: Number(disc.invalidProperty || disc.invalid_property_cnt) || 0,
          mainProperty: {
            // 转换为 MainProperty 类型
            name: disc.mainProperty?.name || disc.mainProperty?.property_name || '',
            value: String(disc.mainProperty?.value || disc.mainProperty?.val || '')
          },
...
const switchCharacter = (characterName) => {
  ...
  try {
    ...
    // 确保所有6个位置都有数据
    const discDetails = [];
    const existingPositions = new Set(targetCharacter.driveDiscs.map(disc => disc.position));
