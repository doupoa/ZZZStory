...
function calculateDriveDiscScore(
  ...
  validSubProperties.forEach((prop) => {
    ...
    // 输出调试信息
    console.log(
      `位置${driveDisc.position} - 副词条: ${prop.name}(${prop.value}) -> 匹配: ${propertyName}, 权重: ${weight}, add: ${addValue}, 加权值: ${weightedValue}`,
    );
  });
  ...
  // 5.1 排序副词条权重（从高到低）
  const availableSubWeights = SUB_STATS_POOL.filter(
    (stat) => weightConfig[stat] > 0,
  )
  ...
  if (position >= 4 && position <= 6) {
    ...
    for (const mainStat of allMainStats) {
      ...
      // 过滤掉与主属性名称相同的副词条
      const filteredSubWeights = availableSubWeights.filter(
        ([stat]) => stat !== mainStat,
      );

      // 计算当前主属性下副词条的最大权重
      const currentSubStatsWeight =
        (filteredSubWeights[0]?.[1] || 0) +
  ...
  // 输出调试信息
  console.log(
    `位置${position} - 副词条最大: ${subPropertyMax.toFixed(4)}, 主属性最大: ${mainPropertyMax.toFixed(4)}, 总和: ${maxWeightSum.toFixed(4)}`,
  );
  ...
  // 输出调试信息
  console.log(
    `位置${position} - 实际得分: ${actualScore.toFixed(4)} (副词条权重: ${subPropertiesWeight.toFixed(4)} + 主属性权重: ${mainPropertyWeight.toFixed(4)}) × 每权重分值: ${scorePerWeight.toFixed(4)} × 品质权重: ${qualityWeight.toFixed(4)}`,
  );
...
function calculateCharacterTotalScore(
  ...
  // 确保driveDiscs是一个数组
  const driveDiscs = Array.isArray(characterData.driveDiscs)
    ? characterData.driveDiscs
    : [];
