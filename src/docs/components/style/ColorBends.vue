<!-- Vue Bits - https://vue-bits.dev/backgrounds/color-bends -->
<template>
...
void main() {
  ...
  // Mix with background image if enabled
  if (uUseBackground > 0) {
    vec4 bgColor = texture2D(uBackgroundTexture, vUv);
    // Background at bottom, ColorBends on top with transparency
    vec3 finalColor = mix(bgColor.rgb, rgb, a);
...
const textureRef = ref<THREE.Texture | null>(null);
const textureLoaderRef = ref<THREE.TextureLoader | null>(null);
...
const setup = () => {
  ...
  const material = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uTime: { value: 0 },
      uTimeOffset: { value: Math.random() * 1000 },
      ...
      uUseBackground: { value: props.backgroundImage ? 1 : 0 }
    },
...
watch(
  ...
  () => {
    ...
    if (props.backgroundImage && material.uniforms.uBackgroundTexture.value) {
      const textureLoader = textureLoaderRef.value;
      if (textureLoader) {
        const oldTexture = material.uniforms.uBackgroundTexture.value as THREE.Texture;
        if (oldTexture) oldTexture.dispose();
        
        const newTexture = textureLoader.load(props.backgroundImage);
        newTexture.wrapS = THREE.ClampToEdgeWrapping;
        newTexture.wrapT = THREE.ClampToEdgeWrapping;
        newTexture.minFilter = THREE.LinearFilter;
        newTexture.magFilter = THREE.LinearFilter;
        material.uniforms.uBackgroundTexture.value = newTexture;
        textureRef.value = newTexture;
      }
    }
    material.uniforms.uUseBackground.value = props.backgroundImage ? 1 : 0;
  },
  { deep: true }
);
</script>
